...
class TwoStreamSVT_CLIP(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if SVT_EMBED_DIM % int(num_heads) != 0:
            raise ValueError(
                f"Fixed SVT embed dim {SVT_EMBED_DIM} must be divisible by num_heads={num_heads}."
            )
    ...
    def _to_svt_input(
        ...
        x_bcthw = torch.cat([mhi, flow], dim=1)
        return x_bcthw.permute(0, 2, 1, 3, 4).contiguous()
