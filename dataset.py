...
from augment import select_flow_mhi_indices, random_spatial_motion
from util import list_videos, classnames_from_id_csv
...
class MotionTwoStreamZstdDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        try:
            ...
            mhi, second = random_spatial_motion(
                mhi, second, rng,
...
def aligned_indices_from_superset(flow_idx: torch.Tensor, mhi_frames: int) -> torch.Tensor:
...
def compute_mhi_and_flow_stream(
    ...
    *,
    ...
    mhi_idx = aligned_indices_from_superset(flow_idx, mhi_frames).cpu()

    flow_set = set(map(int, flow_idx.tolist()))
