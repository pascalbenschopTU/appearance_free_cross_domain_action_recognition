...
## Custom Dataset Manifests (NTU / RWF2000 examples)

You can generate manifests for custom RGB datasets with:

```bash
# 1) NTU60 (xsub / xview)
python scripts/data/build_ntu60_manifests.py \
  --root_dir /PATH/TO/NTU/nturgb+d_rgb \
  --split xsub \
  --out_dir datasets_splits/custom \
  --out_prefix ntu60_xsub \
  --labels_out labels/custom/ntu60_labels.csv

python scripts/data/build_ntu60_manifests.py \
  --root_dir /PATH/TO/NTU/nturgb+d_rgb \
  --split xview \
  --out_dir datasets_splits/custom \
  --out_prefix ntu60_xview \
  --labels_out labels/custom/ntu60_labels.csv

# K=16 few-shot train manifest from NTU60 xsub train split
python scripts/data/build_ntu60_manifests.py \
  --root_dir /PATH/TO/NTU/nturgb+d_rgb \
  --split xsub \
  --out_dir datasets_splits/custom \
  --out_prefix ntu60_xsub_k16 \
  --samples_per_class 16 \
  --seed 0 \
  --labels_out labels/custom/ntu60_labels.csv

# NTU120 eval subset (actions 61..120) from supplemental folder
python scripts/data/build_ntu60_manifests.py \
  --root_dir /PATH/TO/NTU/nturgb+d_rgb_s018/nturgb+d_rgb \
  --split all \
  --action_min 61 \
  --action_max 120 \
  --out_dir datasets_splits/custom \
  --out_prefix ntu120_61_120 \
  --labels_out labels/custom/ntu120_61_120_labels.csv
```

```bash
# 2) RWF2000-style class folders (root/Fight/*, root/NonFight/*)
python scripts/data/build_classfolder_manifests.py \
  --root_dir /PATH/TO/RWF2000 \
  --out_dir datasets_splits/custom \
  --manifest_prefix rwf2000 \
  --labels_out labels/custom/rwf2000_labels.csv \
  --val_ratio 0.2 \
  --seed 0
```

This writes:
- `datasets_splits/custom/*_train.txt`
- `datasets_splits/custom/*_val.txt`
- `datasets_splits/custom/*_all.txt`
- labels CSV (`id,name`)
