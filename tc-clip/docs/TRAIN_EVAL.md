...
### Custom RGB examples (NTU / RWF2000)

Use `resume=/PATH/TO/PRETRAINED_OR_FINETUNED_CHECKPOINT.pth` to load pretrained TC-CLIP weights for finetuning or evaluation.

```bash
# Generic custom dataset finetune with your own manifests
torchrun --nproc_per_node=4 main.py -cn fully_supervised \
data=fully_supervised_custom \
custom.root=/PATH/TO/DATASET_ROOT \
custom.num_classes=NUM_CLASSES \
custom.label_file=/PATH/TO/labels.csv \
custom.train_ann_file=/PATH/TO/train_manifest.txt \
custom.val_ann_file=/PATH/TO/val_manifest.txt \
custom.test_ann_file=/PATH/TO/test_manifest.txt \
output=workspace/expr/fully_supervised_custom/tc_clip \
trainer=tc_clip
```

```bash
# Few-shot finetune on NTU60 K=16 from zero-shot K400 pretrained checkpoint
torchrun --nproc_per_node=4 main.py -cn few_shot \
data=few_shot_ntu60 \
shot=16 \
resume=/PATH/TO/zero_shot_k400_tc_clip.pth \
ntu60.root=/PATH/TO/NTU/nturgb+d_rgb \
output=workspace/expr/few_shot/ntu60_k16_tc_clip \
trainer=tc_clip
```

```bash
# Evaluate the finetuned checkpoint on NTU120 actions 61..120 and RWF2000
torchrun --nproc_per_node=4 main.py -cn zero_shot \
data=zero_shot_eval_ntu12061_rwf2000 \
eval=test \
resume=workspace/expr/few_shot/ntu60_k16_tc_clip/best.pth \
ntu60.root=/PATH/TO/NTU/nturgb+d_rgb \
ntu120_61_120.root=/PATH/TO/NTU/nturgb+d_rgb_s018/nturgb+d_rgb \
rwf2000.root=/PATH/TO/RWF2000 \
output=workspace/results/ntu60_k16_eval_ntu12061_rwf2000 \
trainer=tc_clip
```

```bash
# One script that does both
bash scripts/train/custom/train_eval_tc_clip_ntu60_k16_to_ntu120_rwf2000.sh
```
